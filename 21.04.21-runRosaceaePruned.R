...
for(iter in 1:10){
  ...
  for(j in 1:length(data)){
    file.name <- paste0(wd, "/simmaps_pruned/", labels[4], "-", names(data)[j], "-", format(Sys.time(), "%y_%m_%d"), "-simmap-", iter, ".Rsave")
    simmaps <- getSimmaps(file = Rsaves[4], dat = data[[j]], save.file = file.name, nMap = nmap)
    if(dim(data[[j]])[2] == 4){
      mserr = "known"
    }else{
      mserr = "none"
    ...
    for(k in 1:length(models)){
      ...
      file.name <- paste0(wd, "/res_ouwie_pruned/", labels[4], "/", labels[4], "-", names(data)[j], "-", format(Sys.time(), "%y_%m_%d"), "-OURes-", models[k], "-", iter, ".Rsave")
      ...
      obj.CID <- mclapply(simmaps, function(x) singleRunCID(data[[j]], x, models[k], mserr), mc.cores = ncores)
      file.name.CID <- paste0("/space_2/jamesboyko/2021_SeedDispersal/res_CID_pruned/", labels[4], "/", labels[4], "-", names(data)[j], "-", format(Sys.time(), "%y_%m_%d"), "-CID-", models[k], "-", iter, ".Rsave")
      save(obj.CID, file = file.name.CID)
      obj.CD <- mclapply(simmaps, function(x) singleRunCD(data[[j]], x, models[k], mserr), mc.cores = ncores)
      file.name.CD <- paste0("/space_2/jamesboyko/2021_SeedDispersal/res_CID_pruned/", labels[4], "/", labels[4], "-", names(data)[j], "-", format(Sys.time(), "%y_%m_%d"), "-CD-", models[k], "-", iter, ".Rsave")
...
## combine all the different model runs into one per simmap

ComeTogether <- function(name.clade, name.dat){
  cat("Starting", name.clade, "for", name.dat, "...\n")
  folder.path.a <- paste0("/space_2/jamesboyko/2021_SeedDispersal/res_CID_pruned/", name.clade)
  Rsaves.a <- dir(folder.path.a, full.names = TRUE)
  Rsaves.a <- Rsaves.a[grep(paste0("-dat.", name.dat, "-"), Rsaves.a)]
  folder.path.b <- paste0("/space_2/jamesboyko/2021_SeedDispersal/res_ouwie_pruned/", name.clade)
  Rsaves.b <- dir(folder.path.b, full.names = TRUE)
  Rsaves.b <- Rsaves.b[grep(paste0("-dat.", name.dat, "-"), Rsaves.b)]
  map.files <- dir("/space_2/jamesboyko/2021_SeedDispersal/simmaps_pruned/")
  ...
  for(iter in 1:10){
    ...
    for(i in 1:100){
      ...
      save(out, file = paste0("/space_2/jamesboyko/2021_SeedDispersal/res_bymap_pruned/", name.clade, "/", file.name))
    }
  }
}
...
#   ComeTogether(name.clade = "Rosaceae", name.dat = dat.type)
# }
require(parallel)
mclapply(dat.types, function(x) ComeTogether(name.clade = "Rosaceae", name.dat = x), mc.cores = 8)
...
getSummaryTables <- function(name.clade, name.dat){
  folder.path <- paste0("/space_2/jamesboyko/2021_SeedDispersal/res_bymap_pruned/", name.clade)
  ...
  cat("\nSaving Tables. \n")
  file.name <- paste0("/space_2/jamesboyko/2021_SeedDispersal/res_tables_pruned/", name.clade, "-", name.dat, ".Rsave")
...
getSummaryTablesNames <- function(name.clade, name.dat){
  ...
  cat("\nSaving Table Names. \n")
  file.name <- paste0("/space_2/jamesboyko/2021_SeedDispersal/res_tables_pruned/names-", name.clade, "-", name.dat, ".Rsave")
...
require(parallel)
mclapply(dat.types, function(x) getSummaryTables(name.clade = "Rosaceae", name.dat = x), mc.cores = 8)
mclapply(dat.types, function(x) getSummaryTablesNames(name.clade = "Rosaceae", name.dat = x), mc.cores = 8)
...
Rsaves <- dir("~/2021_SeedDispersal/res_tables_pruned/", full.names = TRUE)
...
for(i in 1:length(Rsaves_res)){
  ...
  FileName <- gsub(".Rsave", "-PrunedTipAvgTable.csv", FileName)
